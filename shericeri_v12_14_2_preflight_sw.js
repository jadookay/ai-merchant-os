/* Shericeri V12.14.2 Service Worker — offline-first web shell */
const CACHE = 'shericeri-v12.14.2-preflight-v1';
...
self.addEventListener('install', (event) => {
  event.waitUntil((async () => {
    ...
    for (const url of STATIC_REMOTE) { try { await cache.add(url); } catch (_) {} }
...
self.addEventListener('activate', (event) => {
  event.waitUntil((async () => {
    ...
    await Promise.all(keys.filter((key) => key.startsWith('shericeri-v12') && key !== CACHE).map((key) => caches.delete(key)));
  ...
  if (request.mode === 'navigate') {
    event.respondWith(fetch(request).then((response) => {
      if (response && response.ok) {
        event.waitUntil(caches.open(CACHE).then((cache) => cache.put(APP, response.clone())).catch(() => {}));
      }
      return response;
  ...
  if (knownRemote) {
    event.respondWith(fetch(request).then((response) => {
      if (response && (response.ok || response.type === 'opaque')) {
        event.waitUntil(caches.open(CACHE).then((cache) => cache.put(request, response.clone())).catch(() => {}));
      }
      return response;
