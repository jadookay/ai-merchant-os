const CACHE='shericeri-v12.6-voice-loop-v1';
...
self.addEventListener('fetch',event=>{
  ...
  if(req.mode==='navigate'){
    event.respondWith(caches.match(APP).then(cached=>cached||fetch(req).catch(()=>caches.match(APP))));
    return;
  }
  ...
  event.respondWith(caches.match(req).then(cached=>cached||fetch(req).then(resp=>{
    if(resp&&(resp.ok||resp.type==='opaque'))caches.open(CACHE).then(c=>c.put(req,resp.clone())).catch(()=>{});
    return resp;
