const CACHE = 'shericeri-v12.11.2-preflight-v1';
...
self.addEventListener('fetch', event => {
  ...
  // Never intercept authenticated/backend requests.
  if (isSupabaseApi(req.url) || req.headers.has('Authorization')) return;

  // Navigation is network-first so a newly deployed index.html wins over
  // an old cached shell. The cache is used only as an offline fallback.
