```js
const CACHE='shericeri-v12.2-android-audio-v1';
  ...
  event.waitUntil((async () => {
    ...
    for (const url of STATIC_REMOTE) {
      try {
        ...
        // Optional dependency; do not fail installation.
      }
    }
...
self.addEventListener('activate', event => {
  event.waitUntil((async () => {
    ...
    await Promise.all(
      keys
        .filter(k => k.startsWith('shericeri-v12') && k !== CACHE)
...
self.addEventListener('fetch', event => {
  ...
  // Never cache authenticated/backend responses.
  if (isSupabaseApi(req.url) || req.headers.has('Authorization')) return;

  // App navigation: use cached shell first, network second.
  if (req.mode === 'navigate') {
    event.respondWith(
      caches.match(APP).then(
        cached => cached || fetch(req).catch(() => caches.match(APP))
      )
    );
    return;
  }

  // Only cache same-origin assets and the explicit public dependencies.
